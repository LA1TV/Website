...
require("imports?this=>window!exports?plupload!lib/plupload");
require("./ajax-upload.css");
...
var AjaxUpload = function(allowedExtensions, uploadPointId, stateParam, removeEnabled) {
	...
	function updateProcessState() {
		...
		$.ajax(PageData.get("baseUrl")+"/admin/upload/processinfo", {
			cache: false,
			dataType: "json",
	...
	function doRemoteRemove(id) {
		if (id !== remoteId) {
			...
			$.ajax(PageData.get("baseUrl")+"/admin/upload/remove", {
				cache: false,
				dataType: "json",
				headers: AjaxHelpers.getHeaders(),
				data: {
